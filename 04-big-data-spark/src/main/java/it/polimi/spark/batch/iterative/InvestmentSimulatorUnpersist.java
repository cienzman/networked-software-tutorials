...
public class InvestmentSimulatorUnpersist {
    ...
    public static void main(String[] args) {
        ...
        while (sum < threshold) {
            ...
            /*
             * - Workers provide caches resources that can be in memory or in disk space.
             * - The method cache() caches the RDD in the main memory (default) but there are also customize version of cache() to store in memory/disk or 
             * 		serialize/not serialize.
             * - When worker have no more resources available some RDDs need to be prune from the cache with the LRU policy
             * - So in this version there is a better utilization of resources because we cache only the previous iteration (the one that is necessary to 
             * 		to compute the new iteration, while all the other are unpersisted.
             */
            investments.cache();
