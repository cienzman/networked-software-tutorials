...
public class SensorDataProcessor {
	...
	public static void main(String[] args) {
		...
		// Create Temperature sensor actors and pass the dispatcher to them
		List<ActorRef> sensors = new LinkedList<ActorRef>();
		for (int i = 0; i < NO_SENSORS; i++) {
			ActorRef ts = sys.actorOf(TemperatureSensorActor.props(), "t" + i);
			sensors.add(ts);
			ts.tell(new ConfigSensorMsg(dispatcher), ActorRef.noSender());
		}
		...
		faultySensor.tell(new ConfigSensorMsg(dispatcher), ActorRef.noSender());
