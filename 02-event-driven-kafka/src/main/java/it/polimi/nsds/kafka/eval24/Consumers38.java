package it.polimi.nsds.kafka.eval24;
...
// and the number of instances of each Consumer
			/* ####1
			 * Number of partitions for inputTopic:
			 * 	CONSUMER 1: in order to make consumer1 sum all messaged the important think is that it is the only consumer in its gruop.
			 * 				it can also have more instances but he must be the only consumer in its group to receive all mex.
			 * CONSUMER 2:
			 * 	
			 * ####2
			 * Number of partitions for outputTopic1 (min, max):
			 * CONSUMER1:
			 *  Only Consumer1 writes to this topic. No consumer reads it Partition count does not affect correctness, only throughput.
			 *     
			 *     
			 * ####3
			 * Number of partitions for outputTopic2 (min, max):
            ...
            while (true) {
                ...
                for (final ConsumerRecord<String, Integer> record : records) {
                    ...
                    if (windowCount.get(key) == 10) {
                        System.out.println("Window full for key "+key);
