...
public class BasicProducer {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < numMessages; i++) {
            ...
            final String key = "Key" + r.nextInt(2);
            ...
            /*
             * Kafka producer allows the configuration so that message delivery can be acknowledged or not.
             * 		- AT MOST ONCE: If the configuration is such that the producer will not be getting an acknowledgment of the message delivery
             * 	       then it will be At most once semantics. There is no guarantee that the message will be committed to the broker or not
             * 		   and the producer will not be attempting to resend the message.
             * 		- AT LEAST ONCE: If the producer can wait for the message to be committed then producer can know if the message was sent successfully or not. 
             *         So in the absence of the acknowledgment, a producer can retry sending the message.     
             */
            if (waitAck) {
                try {
